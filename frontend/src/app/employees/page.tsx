...
export default function EmployeesPage() {
  ...
  const handleToggleActive = async (id: string, currentStatus: boolean) => {
    ...
    const loadingToast = toast.loading(getText('Updating status...', 'ሁኔታ እየተዘመነ ነው...'));
    ...
    try {
      ...
      // Update local state
      setTeachers(prev => prev.map(t => t._id === id ? { ...t, isActive: !currentStatus } : t));
      setFilteredTeachers(prev => prev.map(t => t._id === id ? { ...t, isActive: !currentStatus } : t));
    } catch (error: any) {
      toast.dismiss(loadingToast);
  ...
  const exportToPDF = () => {
    ...
    // Prepare table data
    const tableData = filteredTeachers.filter(t => t.isActive !== false).map(teacher => [
      teacher.teacherId,
      teacher.fullName,
      teacher.phone,
      teacher.role,
                      ...
                      <td colSpan={6} className="px-6 py-12 text-center">
                        <div className="flex flex-col items-center">
                          ...
                          <p className={`text-sm ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>
                            {searchTerm ? getText('Try adjusting your search', 'ፍለጋዎን ማስተካከል ይሞክሩ') : getText('Add your first employee', 'የመጀመሪያውን ሰራተኛ ያክሉ')}
                          </p>
                        </div>
                      </td>
                    </tr>
                  ) : (
                    filteredTeachers.filter(t => t.isActive !== false).map((teacher) => (
                      <tr key={teacher._id} className={`${theme === 'dark' ? 'hover:bg-gray-700' : 'hover:bg-gray-50'}`}>
                        ...
                        <td className="px-3 py-3">
                          <div className="flex gap-1">
                            ...
                            <button
                              onClick={() => router.push(`/employees/edit/${teacher._id}`)}
                              className={`text-xs px-2 py-1 rounded transition-colors ${theme === 'dark' ? 'bg-yellow-800 text-yellow-200 hover:bg-yellow-700' : 'bg-yellow-100 text-yellow-700 hover:bg-yellow-200'}`}
                              title={getText('Edit', 'አርትዕ')}
                            >
                              {getText('Edit', 'አርትዕ')}
                            </button>
                            <button
                              onClick={() => handleToggleActive(teacher._id, teacher.isActive !== false)}
                              className={`text-xs px-2 py-1 rounded transition-colors ${teacher.isActive !== false ? (theme === 'dark' ? 'bg-orange-800 text-orange-200 hover:bg-orange-700' : 'bg-orange-100 text-orange-700 hover:bg-orange-200') : (theme === 'dark' ? 'bg-green-800 text-green-200 hover:bg-green-700' : 'bg-green-100 text-green-700 hover:bg-green-200')}`}
                              title={teacher.isActive !== false ? getText('Deactivate', 'አቦዝን') : getText('Activate', 'አንቃ')}
                            >
                      ...
                      <div className="flex items-start space-x-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex gap-2 mt-3">
                            ...
                            <button
                              onClick={() => router.push(`/employees/edit/${teacher._id}`)}
                              className={`text-xs px-3 py-1.5 rounded-md font-medium transition-colors ${theme === 'dark' ? 'bg-yellow-800 text-yellow-200 hover:bg-yellow-700' : 'bg-yellow-100 text-yellow-700 hover:bg-yellow-200'}`}
                            >
                              {getText('Edit', 'አርትዕ')}
                            </button>
                            <button
                              onClick={() => handleToggleActive(teacher._id, teacher.isActive !== false)}
                              className={`text-xs px-3 py-1.5 rounded-md font-medium transition-colors ${teacher.isActive !== false ? (theme === 'dark' ? 'bg-orange-800 text-orange-200 hover:bg-orange-700' : 'bg-orange-100 text-orange-700 hover:bg-orange-200') : (theme === 'dark' ? 'bg-green-800 text-green-200 hover:bg-green-700' : 'bg-green-100 text-green-700 hover:bg-green-200')}`}
                            >
                              {teacher.isActive !== false ? getText('Inactive', 'አቦዝን') : getText('Active', 'አንቃ')}
                            </button>
                            <button
