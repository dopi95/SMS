...
export default function EditStudentPage() {
  ...
  const fetchStudent = async () => {
    try {
      ...
      setFormData({
        ...
        middleNameAmharic: student.middleNameAmharic || '',
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    if (name === 'studentId') {
      // Extract year from student ID and update joined year
      const match = value.match(/BLUE\d{3}\/(\d{4})/);
      if (match) {
        const year = match[1];
        setFormData(prev => ({ ...prev, [name]: value, joinedYear: year }));
        return;
      }
    }
    
    if (name === 'joinedYear' && formData.studentId && value.length === 4) {
      // Update student ID with new year while keeping the same number
      const match = formData.studentId.match(/BLUE(\d{3})\/(\d{4})/);
      if (match) {
        const number = match[1];
        const newStudentId = `BLUE${number}/${value}`;
        setFormData(prev => ({ ...prev, [name]: value, studentId: newStudentId }));
        return;
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    const loadingToast = toast.loading(getText('🔄 Updating student information...', '🔄 የተማሪ መረጃ እየተዘመነ ነው...'));
    ...
    try {
      ...
      toast.success(getText('✅ Student updated successfully! Redirecting...', '✅ ተማሪ በተሳካ ሁኔታ ተዘምኗል! እየተዛወረ ነው...'));
      ...
      toast.dismiss(loadingToast);
      const errorMessage = error.response?.data?.message || getText('Failed to update student', 'ተማሪን ማዘመን አልተሳካም');
          ...
          <div className={`rounded-lg shadow-sm border ${theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
            <div className={`px-6 py-4 border-b ${theme === 'dark' ? 'border-gray-700' : 'border-gray-200'}`}>
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h1 className={`text-2xl font-bold ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>{getText('Edit Student', 'ተማሪ ያርትዑ')}</h1>
                  <p className={`mt-1 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>{getText('Update student information', 'የተማሪ መረጃ ያዘምኑ')}</p>
                </div>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-8">
              ...
              {/* Student Information */}
              <div>
                <h3 className={`text-lg font-semibold mb-4 ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>{getText('Student Information', 'የተማሪ መረጃ')}</h3>
              ...
              <div>
                <h3 className={`text-lg font-semibold mb-4 ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>{getText('Additional Information', 'ተጨማሪ መረጃ')}</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className={`block text-sm font-medium mb-2 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>
                      {getText('Email', 'ኢሜይል')}
              ...
              <div className={`flex justify-end gap-4 pt-6 border-t ${theme === 'dark' ? 'border-gray-700' : 'border-gray-200'}`}>
                <button
                  type="button"
                  onClick={() => router.back()}
                  className={`px-6 py-2 border rounded-lg transition-colors ${theme === 'dark' ? 'border-gray-600 text-gray-300 hover:bg-gray-700' : 'border-gray-300 text-gray-700 hover:bg-gray-50'}`}
                >
                  {getText('Cancel', 'ይሰርዙ')}
                </button>
                <button
                ...
                >
                  ...
                  {loading ? getText('Updating Student...', 'ተማሪ እየተዘመነ ነው...') : getText('Update Student', 'ተማሪ ያዘምኑ')}
                </button>
              </div>
            </form>
          </div>
